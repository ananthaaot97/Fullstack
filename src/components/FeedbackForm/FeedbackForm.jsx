...
export default function FeedbackForm({ resource, onClose }) {
  ...
  // ── Ratings context: shared store persisted to localStorage ──
  const { getReviews, getAvgRating, addReview } = useRatings();
  ...
  // Derive reviews and stats from shared context (re-computes on every new review)
  ...
  const handleSubmit = async (e) => {
    ...
    // Persist to shared context (localStorage) so admin dashboard reflects it
    addReview(resource.id, newReview);
    setLoading(false);
    setSubmitted(true);
  };
